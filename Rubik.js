...
const raycastMouseMove = (e) => {
  ...
  if (rotationDirection !== undefined && startSelectedCube !== undefined) {
    ...
    if (rotationDirection === "y" || rotationDirection === "-y") {
      group = cubes.filter(
        (x) =>
          Math.abs(x.position.y - startSelectedCube.object.position.y) < 0.05
      );
    } else if (rotationDirection === "x" || rotationDirection === "-x") {
      group = cubes.filter(
        (x) =>
          Math.abs(x.position.x - startSelectedCube.object.position.x) < 0.05
      );
    } else if (rotationDirection === "z" || rotationDirection === "-z") {
      group = cubes.filter(
        (x) =>
          Math.abs(x.position.z - startSelectedCube.object.position.z) < 0.05
      );
    }
...
// const distanceCompute = (e) => {
//   if (rotatingCubes === undefined) return;

//   console.log(rotatingCubes);
//   mouse = { x: 0, y: 0 };
//   mouse.x = (e.clientX / window.innerWidth) * 2 - 1;
//   mouse.y = -(e.clientY / window.innerHeight) * 2 + 1;
...
//   distance = Math.sqrt(
//     Math.pow(vectorDistance.x, 2) + Math.pow(vectorDistance.y, 2)
//   );
...
// window.addEventListener("mousemove", distanceCompute);
...
document.getElementById("reset-view").addEventListener("touchstart", () => {
  let currentPosition = camera.position;
  let targetPosition = { x: 5, y: 5, z: 5 };
  let duration = 2000;
  let stepX = ((targetPosition.x - currentPosition.x) / duration) * 10;
